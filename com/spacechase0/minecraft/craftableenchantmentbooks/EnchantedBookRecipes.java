    ...
    private ItemStack findMatchingRecipe( InventoryCrafting inv )
    {
    	for ( int r = 0; r < recipes.size(); ++r )
    	{
	        ...
	        for ( int i = 0; i < inv.getSizeInventory(); ++i )
	        {
            	...
            	if ( !hasBook && recipe.in == null && stack.getItem() == Item.book )
            	{
            		hasBook = true;
            	}
            	else if ( !hasBook && stack.getItem() == Item.enchantedBook && recipe.in != null )
            	{
            		ItemStack in = Item.enchantedBook.getEnchantedItemStack( recipe.in );
            		NBTTagList inTag = in.getTagCompound().getTagList( "StoredEnchantments" );
            		if ( stack.isItemEqual( in ) && stack.getTagCompound().getTagList( "StoredEnchantments" ).equals( inTag ) )
            		{
            			hasBook = true;
            		}
            	}
	    	...
	    	if ( found )
	    	{
	    		return Item.enchantedBook.getEnchantedItemStack( recipe.out );
	    	}
    	}
